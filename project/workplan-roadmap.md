...
> CDR(Critical Design Review) 설계 반영 항목은 [cdr-design-checklist.md](cdr-design-checklist.md) 참조
...
- **서비스 분석 우선순위 결정** — PM+PO 합의로 30개 서비스 분석 순서 확정 (manual-tasks.md P0-1)
- **Baseline Commit 확정** — 서비스별 분석 기준 브랜치/커밋 확정 (manual-tasks.md P0-4)
- **기존 설계 문서 수집** — 서비스별 기존 요구사항/설계/운영 문서 수집 (manual-tasks.md P0-3)
- **문서 인프라 결정** (아래 상세)
...
- **서비스 분석 우선순위표** (30개 서비스 분석 순서, 판단 기준)
- **Baseline Commit 목록** (서비스별 분석 기준 브랜치/커밋)
...
- 기술 부채 마커 추출 (TODO/FIXME/HACK 전수 스캔)
- IaC(Infrastructure as Code) 분석
- CI/CD 파이프라인 상세 분석
- 화면 목록 추출 및 화면명 정규화
- API 엔드포인트 자동 추출
- DB 스키마 역추적
...
- 유저 시나리오(Use Case) 추출 — B2C 공통 Use Case 매핑 + 서비스 특화 Use Case 발굴
- 유저 시나리오별 비기능(NFR) 체크포인트 연계 — 보안/품질/성능 교차 매핑
- 비기능 현황 수집 (NFR Baseline)
- **런타임 트래픽 데이터 수집** (수작업 — APM/모니터링에서 TPS, 응답시간, 에러율 수집, manual-tasks.md P1-1~P1-3)
...
- 2-4. 이벤트/메시지 흐름 & E2E Call Flow View → PM, 개발자
- 2-4b. E2E Call Flow 완전성 검증 보강 (핵심 Journey 9개, 멀티 Repo 교차 검증) → PM, 개발자, 품질센터
- 2-5. 데이터 View → 개발자, 보안센터
- 2-6. UI/UX View → UX센터, PO
- 2-7. 보안 View → 보안센터
- 2-8. 품질 View → 품질센터
- 2-9. STB 하드웨어 리소스 관리 View → PM, 개발자, 품질센터
- 2-10. 장애 복원력 및 DR View → PM, 개발자, 품질센터 [CDR 11장]
- 2-11. AI 거버넌스 View → PM, 개발자, 품질센터 [CDR 12장]
- 2-12. Layer Stack View (기술 계층 구조) → PM, 개발자, 인프라팀
- 2-13. Function–External Stack Call Flow View → PM, 개발자
- 2-14. PM 의사결정 지원 다이어그램 → PM, 개발 리드
...
- 데이터 정합성 위험 시나리오 검증
- STB 리소스 Gap 분석
- **서비스 간 교차 검증** — API 계약/이벤트 스키마/용어 일관성 검증 (manual-tasks.md P3-1)
- **서비스 합리화 판단** — 기능 중복/저사용/고비용 서비스 → 통합/폐기/재작성 후보 (manual-tasks.md P3-5)
- 용어 사전 최종 검증
