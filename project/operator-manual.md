...
## 10. 배포 Pre-Flight 체크리스트

> 산출물을 Stakeholder에게 배포하거나, 프로젝트 의사결정에 활용하기 전에 아래 항목을 점검하세요.
> 불완전한 산출물이 배포되면 잘못된 의사결정으로 이어질 수 있습니다.

### 10-1. 산출물 완전성 점검

| # | 점검 항목 | 확인 방법 | 통과 기준 |
|---|---|---|---|
| 1 | Phase 1 전체 Step 완료 여부 | `service-inventory.md` 진행 현황 테이블 | Step 01~08 모두 ✅ |
| 2 | Phase 2 View 생성 완료 여부 | `services/{서비스명}/docs/views/` 하위 폴더 확인 | 15개 View 섹션 산출물 존재 |
| 3 | Phase 3 Gap 분석 완료 여부 | `services/{서비스명}/docs/gap-analysis/` 확인 | Gap 리포트 존재 |
| 4 | 확인 상태 미표기 항목 없음 | 산출물 내 ✅/⚠️/❌/🔍 태그 전수 확인 | 상태 미표기 항목 0건 |
| 5 | 🔍(인터뷰필요) 항목 해소율 | 산출물 내 🔍 태그 검색 | 핵심 항목 80% 이상 해소 |
| 6 | 신뢰도 등급 확인 | 각 산출물 하단 자가진단 리포트 | 🔴 LOW 산출물 0건 |

### 10-2. 데이터 정합성 점검

| # | 점검 항목 | 확인 방법 | 통과 기준 |
|---|---|---|---|
| 7 | 화면 ↔ API 매핑 완전성 | Phase 1 중간 교차 검증 결과 확인 | 고아 화면/고아 API 0건 또는 사유 기재 |
| 8 | API ↔ Use Case 매핑 완전성 | Use Case ↔ Call Flow 교차 매핑 테이블 | 미매핑 API 0건 또는 사유 기재 |
| 9 | Dead Code 제외 일관성 | Step 1b 판정 vs Phase 2 다이어그램 | Dead 판정 항목이 다이어그램에서 제외됨 |
| 10 | 서비스 간 API 계약 일치 | 2-4b 교차 검증 결과 | 불일치 항목이 모두 ⚠️ 표기됨 |
| 11 | 용어 사전 일관성 | `glossary.md` vs 산출물 내 용어 | 동일 개념에 다른 명칭 사용 0건 |

### 10-3. Stakeholder별 배포 전 점검

| 대상 | 배포 전 확인 사항 |
|---|---|
| PO | Use Case 카탈로그의 시나리오가 `[KIRO+확정]` 태그 항목에 대해 PO 검토 완료되었는가 |
| PM | E2E Call Flow에서 소스 없는 구간(🔍)이 인터뷰로 보완되었는가 |
| 개발자 | 코드 소스 위치(소스 로케이터)가 현재 코드와 일치하는가 (최근 커밋 이후 변경 여부) |
| 보안센터 | 보안 Gap 리포트의 🔴 Critical 항목에 대한 대응 방안이 기재되었는가 |
| 품질센터 | 테스트 커버리지 데이터가 외부 도구(JaCoCo 등) 실행 결과로 보완되었는가 |
| 인프라팀 | 배포 토폴로지가 실제 운영 환경과 일치하는지 인프라팀 확인을 받았는가 |

### 10-4. 배포 직전 최종 체크

- [ ] 모든 산출물이 Git에 커밋/푸시되었는가
- [ ] `_backup/` 폴더에 이전 버전이 보존되어 있는가
- [ ] `_context-notes.md`에 핵심 인사이트와 주의사항이 기록되어 있는가
- [ ] `manual-tasks.md`의 미해소 수작업 항목 목록이 Stakeholder에게 공유되었는가
- [ ] 소스코드 분석 시점(git commit hash)이 산출물에 기록되어 있는가 (코드 변경 시 산출물 갱신 필요 판단 근거)
- [ ] 산출물 내 `[인터뷰]` 태그 항목 중 미해소 건이 "한계 사항"으로 명시되었는가

> ⚠️ 위 체크리스트를 통과하지 못한 산출물은 "Draft" 워터마크를 표기하고,
> Stakeholder에게 "검증 중" 상태임을 명시하여 배포하라.
