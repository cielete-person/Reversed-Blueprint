...
## Phase 1 중간 교차 검증 체크포인트 (Step 04 완료 후)

> Step 04(비즈니스 로직)까지 완료한 시점에서, Step 05~08로 넘어가기 전에 아래 교차 검증을 수행하라.
> 이 체크포인트는 Phase 1 전반부(01~04)의 산출물 정합성을 조기에 확인하여, 후반부(05~08)에서 누적 오류를 방지한다.

- [ ] **화면 ↔ API 매핑 완전성 확인** `[KIRO]`
  - Step 02 화면 인벤토리의 모든 화면이 Step 03 API 엔드포인트와 매핑되는지 확인
  - API 호출이 없는 화면(정적 화면)은 정당한 사유가 있는지 확인
  - 화면에서 호출하는 API가 Step 03 목록에 누락되어 있지 않은지 확인

- [ ] **API ↔ Use Case 매핑 완전성 확인** `[KIRO]`
  - Step 03 API 엔드포인트가 Step 04 Use Case에 매핑되지 않는 "고아 API"가 있는지 확인
  - Use Case에서 참조하는 API가 Step 03 목록에 존재하는지 확인

- [ ] **Dead Code 제외 일관성 확인** `[KIRO]`
  - Step 1b에서 Dead 판정(🔴)된 항목이 Step 02~04 산출물에서 실제로 제외되었는지 확인
  - 조건부 Dead(🟡)가 `⚠️ Dead 의심` 표기와 함께 포함되어 있는지 확인

- [ ] **공통 모듈 참조 일관성 확인** `[KIRO]`
  - Step 1c 공통 모듈이 Step 03(API), Step 04(비즈니스 로직)에서 적절히 구분 표기되었는지 확인

- [ ] **컨텍스트 노트 기록 확인**
  - `_context-notes.md`의 Step 01~04 섹션에 핵심 발견사항이 기록되어 있는지 확인
  - 역방향 피드백 이력이 있으면 해당 산출물이 실제로 업데이트되었는지 확인

> 📌 이 체크포인트에서 불일치가 발견되면, 해당 Step 산출물을 보정한 후 Step 05로 진행하라.
> 📌 KIRO 채팅에서 아래 프롬프트로 자동 검증할 수 있다:
> ```
> Step 01~04 산출물의 교차 검증을 수행해줘. 화면↔API, API↔Use Case 매핑 완전성과 Dead Code 제외 일관성을 확인해줘.
> ```

---

## Phase 1 → Phase 2 전환 게이트 (교차 검증)

> Phase 2를 시작하기 전에 아래 항목을 반드시 확인하세요. 서비스 단위로 병렬 실행된 Phase 1 산출물의 완전성과 일관성을 검증하는 단계입니다.

- [ ] **모든 서비스 Phase 1 산출물 존재 확인** `[KIRO]`
  - `services/{서비스명}/docs/extraction/` 하위에 01~08 폴더별 산출물이 존재하는지 일괄 확인
  - 누락된 서비스/Step이 있으면 Phase 1로 돌아가 보완
- [ ] **서비스 간 API 계약 교차 검증** `[KIRO]`
  - 서버 A의 Feign/RestTemplate/WebClient 호출부 URL ↔ 서버 B의 Controller 엔드포인트 실존 여부 교차 확인
  - 요청/응답 DTO 필드 레벨 일치 여부 (불일치 시 ⚠️ 표기)
  - 인증 토큰 전파 체인 검증 (앱 → GW → 서비스A → 서비스B에서 토큰 유실/변환 구간 식별)
- [ ] **공통 모듈 참조 일관성 확인** `[KIRO]`
  - 각 서비스의 Step 01c 공통 모듈 목록을 횡단 비교하여 동일 모듈의 버전 불일치 식별
- [ ] **용어/화면명 정규화 일관성 확인** `[KIRO]`
  - 각 서비스의 Step 02 화면명 정규화 결과를 횡단 비교하여 동일 화면의 명칭 불일치 식별
- [ ] **컨텍스트 노트 생성 확인**
  - 각 서비스의 `docs/extraction/_context-notes.md`가 존재하고, 핵심 인사이트가 기록되어 있는지 확인
...
- [ ] 주요 Use Case별 E2E Call Flow 시퀀스 다이어그램 `[KIRO]`
  - 핵심 UC(가입, 결제, 인증, 해지 등) 최소 5개 이상 선정
  - 전체 구간 시퀀스: 앱 → API Gateway → 백엔드 서비스 → 외부 시스템 → DB → 응답 → 앱 UI 갱신
  - 정상 흐름 + 에러/fallback 분기 경로 (alt/else 블록)
  - 동기 호출(실선)과 비동기 이벤트(점선) 구분 표기
- [ ] 멀티 Repo 앱↔서버 Call Flow 교차 매핑 `[KIRO]`
  - 앱 코드 API 호출부와 서버 Controller 엔드포인트 교차 매핑
  - 요청/응답 DTO 일치 여부 확인 (불일치 시 ⚠️)
- [ ] Use Case ↔ Call Flow 교차 매핑 테이블 `[KIRO]`
  - UC ID, UC명, 관련 Call Flow ID, 동기/비동기 구분, 주요 외부 연동
- → 대상: PM, 개발자, 품질센터, 보안센터

### 2-4b. E2E Call Flow 완전성 검증 보강 항목
> PM 관점에서 설계도의 Call Flow 단절·누락을 방지하기 위한 추가 검증 항목

- [ ] 크로스 플랫폼 핵심 Journey E2E Call Flow (최소 9개) `[KIRO]`
  - **기존 5개 (플랫폼 횡단)**:
  - ① 인증 플랫폼 횡단 Journey: 앱 로그인 → PASS 인증 → 토큰 발급 → 각 서비스 인가 (모바일/STB 공통)
  - ② 결제 플랫폼 횡단 Journey: 상품 선택(서비스A) → 결제(결제 플랫폼) → 과금(빌링) → 개통(서비스B)
  - ③ 푸시/알림 횡단 Journey: 이벤트 발생(서비스) → 알림 플랫폼 → FCM/APNs → 앱 수신 → 딥링크 → 대상 화면
  - ④ 고객센터 횡단 Journey: 앱 문의 접수 → 고객센터 서비스 → CRM 연동 → 상담 이력 조회
  - ⑤ 콘텐츠 소비 횡단 Journey (미디어): 콘텐츠 탐색 → DRM 인증 → 스트리밍 서버 → CDN → STB/앱 재생
  - **추가 4개 (플랫폼 내부 상세 + 단절 고위험 구간)**:
  - ⑥ STB 콘텐츠 재생 상세 Journey: STB 앱 → TIF(TV Input Framework) → CAS 인증 → DRM 복호화(CADRM) → CDN 스트리밍 → 디코더 → 렌더링
    - CAS/DRM은 별도 벤더 솔루션일 가능성 높음 → 소스 없는 구간 추정 필수
    - STB 기종별(UHD3/UHD4T/TV프리 등) 분기 경로 차이 식별
  - ⑦ IoT 양방향 제어 Journey: 앱 원격 제어 명령 → IoT PAS/SAS → IoT 인증GW → 디바이스 제어 → 상태 변경 이벤트 → PushGW → 앱 UI 갱신
    - 앱→디바이스(제어)와 디바이스→앱(상태 알림) 양방향 경로를 모두 포함
    - CCTV(우리집지킴이): 앱 → P2P 스트리밍 서버 → 영상 스토리지 → 앱 재생 경로 포함
  - ⑧ 메시징 GW 라우팅 Journey: 발송 요청(서비스/기업) → GW 선택 라우팅(국제/050/P2P/사내/기업형) → 통신망 전달 → 수신 확인 콜백
    - SMS GW 5개, MMS 서버 10개 간 내부 라우팅 로직 추적
    - RCS G/W ↔ MaaP FE 간 호출 관계 포함
    - 대부분 소스 없는 구간 → 앞뒤 서비스에서 GW 선택 조건·라우팅 규칙 추정
  - ⑨ 인증 플랫폼 내부 체인 Journey: PASS 앱 → PASS 서버 → CAS 인증 → DAS(U+ID) 조회 → DMS 단말 확인 → 토큰 발급 → 호출 서비스 인가
    - SVC-005~009 간 상호 호출 관계 상세화 (대부분 서버만, 앱 없음)
    - 결제 플랫폼 내부(DCB/소액결제/상품권류)도 유사하게 내부 체인 상세화 필요
  - **공통 원칙**:
  - 각 Journey에서 플랫폼 경계(서비스 간 전환점)를 명시적으로 표기
  - 플랫폼 공유 서버(인증/결제/알림 등)가 여러 서비스에서 호출되는 교차점 식별
  - 소스 없는 구간은 앞뒤 코드 기반 입출력 계약 추정 + 🔍 인터뷰필요 표기
- [ ] 서버 간 내부 API 계약 교차 검증 (멀티 Repo) `[KIRO]`
  - 서비스 A의 Feign Client/RestTemplate 호출 URL과 서비스 B의 Controller 엔드포인트 매칭
  - 서버→서버→서버 내부 호출 체인(3-hop 이상)의 전체 경로 추적
  - 서버 간 요청/응답 DTO 필드 레벨 일치 여부 검증 (서버 A 요청 DTO ↔ 서버 B 수신 DTO, 불일치 시 ⚠️)
  - 인증 토큰 전파 체인 검증: 앱 → GW → 서비스A → 서비스B에서 토큰이 어떻게 전달·변환되는지 추적 (토큰 유실/변환 구간 식별)
  - 호출 체인 상 각 구간의 타임아웃·리트라이·서킷브레이커 설정 일관성 검증
  - 불일치 항목은 ⚠️ 표기하고 `cross-service-inconsistency.md`에 기록
- [ ] 소스 없는 구간 앞뒤 로직 추정 문서 `[KIRO+인터뷰]`
  - 소스 유무 🔍인 서비스(SVC-006~009, SVC-011~013, SVC-016~020, SVC-026~027 등)의 앞뒤 호출 구간 식별
  - 앞단(호출자) 코드에서 요청 파라미터·URL·프로토콜 추출
  - 뒷단(피호출자) 코드에서 응답 파싱·에러 처리 로직 추출
  - 앞뒤 코드 기반으로 소스 없는 구간의 입출력 계약(Contract)을 추정
  - 추정 불가 항목은 🔍 인터뷰필요로 표기하고 manual-tasks.md에 연계
  - **단절 고위험 구간 (소스 없는 서비스가 밀집된 플랫폼)**:
    - 🔴 인증 플랫폼: SVC-006(휴대폰 본인확인), SVC-007(DAS), SVC-008(CAS), SVC-009(DMS) — 4개 서버 모두 앱 없음, 상호 호출 관계가 핵심이나 소스 확보 불확실
    - 🔴 결제 플랫폼: SVC-016(DCB), SVC-017(소액결제), SVC-018~020(상품권류) — 5개 서버 모두 앱 없음, 외부 PG/빌링 연동이 핵심
    - 🔴 메시징 플랫폼: SVC-011(SMS GW 5개), SVC-012(MMS 서버 10개), SVC-013(RCS) — GW 간 내부 라우팅 로직이 복잡
    - 🟡 위치 플랫폼: SVC-025(LBSP 서버 8개), SVC-026(공공위치문자), SVC-027(공항로밍안내문자) — 내부 서버 체인 복잡(SABC→SLP→HPS→WPS→LPS→GMLC 등)
    - 🟡 STB 미들웨어: CAS/DRM/TIF 등 벤더 솔루션 구간 — 소스 없을 가능성 높음
- [ ] OS/네이티브 레이어 ↔ 앱 레이어 경계 Call Flow `[KIRO]`
  - STB: TIF(TV Input Framework) → CAS → DRM 연동 구간에서 OS 레이어와 앱 레이어 간 호출 경로 추적
  - 모바일: 생체인증(BiometricPrompt/LAContext) → Keystore/Keychain → 인증 서버 연동 구간 추적
  - 네이티브(C/C++/Swift/Kotlin) ↔ 앱(JS/Dart/Java) 간 브릿지 호출 경로 식별
  - 한쪽 Repo만으로 추적 불가한 구간은 양쪽 Repo 교차 매핑 또는 🔍 인터뷰필요 표기
- [ ] 배치↔실시간 데이터 흐름 연결도 `[KIRO]`
  - 배치 작업이 생성/변경하는 데이터를 실시간 서비스가 참조하는 경로 식별
  - 실시간 서비스가 생성한 데이터를 배치 작업이 후처리하는 경로 식별
  - 배치 실행 시간대와 실시간 서비스 간 데이터 정합성 위험 구간 식별
  - 배치 지연/실패 시 실시간 서비스에 미치는 영향 범위 매핑
- [ ] 서비스 간 통신 토폴로지 (30개 서비스 전체) `[KIRO]`
  - 30개 서비스 전체의 통신 관계를 단일 토폴로지 다이어그램으로 시각화
  - 통신 프로토콜별 구분: REST(실선), gRPC(굵은 실선), MQ(점선), DB Link(이중선)
  - 통신 빈도별 선 굵기 차등 표기 (고빈도/중빈도/저빈도)
  - 플랫폼 경계(미디어/인증/결제/IoT 등)를 색상 또는 영역으로 구분
...
- [ ] 데이터 오너십 매트릭스 (서비스 × 엔터티) `[KIRO]`
  - 30개 서비스별로 소유(Owner)/읽기(Reader)/변경(Writer) 권한을 가진 데이터 엔터티 매핑
  - 복수 서비스가 동일 엔터티를 변경하는 경우 ⚠️ 충돌 위험 표기
  - Phase 1 DB 스키마 역추적(1-3) 및 서비스 간 API 계약(1-3) 결과 기반
- [ ] 배포 의존성 순서도 `[KIRO]`
  - 서비스 간 배포 순서 제약 식별 (DB 마이그레이션 선행, API 계약 변경 등)
  - 동시 배포 가능 그룹 vs 순차 배포 필수 그룹 구분
  - CI/CD 파이프라인(1-1) 및 서비스 간 호출 관계(1-4) 결과 기반
- [ ] 환경별 구성 차이 매트릭스 `[KIRO]`
  - Dev/Staging/Prod 환경별 주요 설정 차이를 서비스 × 환경 매트릭스로 시각화
  - DB 접속 정보, 외부 연동 URL, 기능 플래그, 로그 레벨, 인스턴스 수 등
  - Phase 1 설정 프로파일 분석(1-1) 결과 기반
- [ ] 서비스 성숙도 레이더 차트 `[KIRO]`
  - 서비스별 6개 축 성숙도 평가: 테스트 커버리지, 보안 준수율, 문서화 수준, 모니터링 수준, 코드 품질, 배포 자동화
  - Phase 1~2 전체 결과를 종합하여 서비스별 레이더 차트 생성
  - 성숙도 낮은 축 = 개선 우선순위 후보
- [ ] 크리티컬 패스 다이어그램 `[KIRO]`
  - 핵심 비즈니스 Journey(가입→결제→개통)의 서비스 호출 체인에서 가장 긴 경로(Critical Path) 식별
...
- [ ] 기술 부채 히트맵 `[KIRO]`
  ...
  - Phase 1 기술 부채 마커(1-1), Dead Code(1-1b), 품질(1-6), 보안(1-7) 결과 종합
- [ ] 온콜/장애 대응 흐름도 `[KIRO+인터뷰]`
...
- [ ] 앱/서버 PM 분리 관리 View `[KIRO]`
  - 앱 PM 섹션: 화면 목록, 앱→서버 API 호출 목록, 앱 쪽 보안/품질 Gap, 앱 외주사 인터페이스 계약 요약
  - 서버 PM 섹션: API 엔드포인트 목록, 서버→외부 연동 목록, 서버 쪽 보안/품질 Gap, 서버 외주사 인터페이스 계약 요약
  - 공통 섹션: 앱↔서버 간 API 계약 일치/불일치 현황, E2E Call Flow에서 앱/서버 경계 표기
  - 멀티 Repo 서비스(앱+서버 분리)에서 특히 유용, 단일 Repo에서도 영역 구분 시 생성 가능
- → 대상: PM, 개발 리드, 인프라팀(배포 의존성/환경별 구성 한정), 앱 PM, 서버 PM
...
- 상태 전이 다이어그램 세트
- 이벤트/메시지 흐름도
- 주요 Use Case별 E2E Call Flow 시퀀스 다이어그램 (앱→Gateway→서비스→외부→DB→응답→앱)
...
- 크로스 플랫폼 핵심 Journey E2E Call Flow (9개: 인증/결제/푸시/고객센터/콘텐츠 횡단 + STB 재생 상세/IoT 양방향/메시징 GW 라우팅/인증 내부 체인)
- 서버 간 내부 API 계약 교차 검증 리포트 (멀티 Repo 3-hop 이상 체인)
- 소스 없는 구간 앞뒤 로직 추정 문서 (입출력 계약 추정, 단절 고위험 구간 🔴/🟡 등급 포함)
- OS/네이티브 레이어 ↔ 앱 레이어 경계 Call Flow (STB TIF/CAS/DRM, 모바일 생체인증/Keystore 브릿지)
- 배치↔실시간 데이터 흐름 연결도
- 서비스 간 통신 토폴로지 다이어그램 (30개 서비스 전체)
- 데이터 오너십 매트릭스 (서비스 × 엔터티, Owner/Reader/Writer)
- 배포 의존성 순서도 (동시 배포 그룹 vs 순차 배포 필수 그룹)
- 환경별 구성 차이 매트릭스 (Dev/Staging/Prod × 서비스)
- 서비스 성숙도 레이더 차트 (6축 평가)
- 크리티컬 패스 다이어그램 (핵심 Journey 최장 경로)
- 기술 부채 히트맵 (서비스 × 부채 유형)
- 온콜/장애 대응 흐름도
- 앱/서버 PM 분리 관리 View (앱 PM 섹션 + 서버 PM 섹션 + 공통 API 계약 일치/불일치 현황)

### Phase 2 → Phase 3 전환 준비: 서비스 요약 인덱스 생성

> Phase 3 횡단 분석 시 30개 서비스의 산출물을 동시에 로드하기 어려우므로, 각 서비스의 핵심 지표를 1줄로 요약한 인덱스 파일을 생성합니다.

- [ ] **서비스 요약 인덱스 생성** `[KIRO]`
  - `docs-integrated/service-summary-index.md`에 30개 서비스의 핵심 지표를 1줄 요약으로 작성
  - 포함 지표: 기술 스택, API 수, 화면 수, 비즈니스 규칙 수, 외부 연동 수, 테스트 커버리지, 보안 Gap 수, Dead Code 비율, 기술 부채 등급
  - 각 서비스의 `_context-notes.md`에서 핵심 인사이트를 추출하여 "주의사항" 컬럼에 기재
  - Phase 3 횡단 분석 시 이 인덱스를 먼저 로드하여 전체 서비스 현황을 파악한 후, 필요한 서비스의 상세 산출물만 선택적으로 로드
