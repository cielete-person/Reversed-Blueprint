...
#### ★ 산출물 순차 저장 원칙 (One-by-One Save)

> 산출물을 여러 개 메모리에 쌓아두고 나중에 한꺼번에 저장하면,
> 컨텍스트 윈도우 한계로 후반부 산출물이 유실되거나 품질이 저하된다.
> 반드시 아래 원칙을 지켜라.

1. **산출물 1개 완성 → 즉시 파일 저장 → 다음 산출물 착수**: 하나의 산출물 파일 작성이 끝나면 반드시 `fsWrite`로 디스크에 저장한 후 다음 산출물을 시작하라
2. **저장 확인 후 진행**: 파일 저장이 성공했는지 확인한 후 다음 산출물로 넘어가라
3. **Part 분할 실행 시에도 동일**: Part 1 내에서도 산출물 A → 저장 → 산출물 B → 저장 → 산출물 C → 저장 순서로 진행하라
4. **대용량 산출물**: 단일 산출물이 200줄 이상이면 `fsWrite`로 앞부분을 먼저 저장하고 `fsAppend`로 나머지를 추가하라
5. **중간 저장 금지 예외 없음**: "나중에 한꺼번에 저장하겠다"는 접근은 절대 금지. 분석 결과가 나오는 즉시 파일로 기록하라
...
#### Step별 산출물 분할 생성 가이드 요약

> 각 Step의 steering 파일에 `🔀 분할 생성 가이드` 섹션이 있다. 아래는 요약 테이블이다.
> 전체 산출물 목록 및 체크리스트: [artifact-checklist.md](../project/artifact-checklist.md)

| Step | 산출물 수 | 분할 전략 | 분할 기준 |
|---|---|---|---|
| 01 코드 구조 | 8개 | 3분할 | 구조스캔 / 설정플랫폼 / 부채인프라 |
| 1b Dead Code | 5개 | 분할 실행 가이드 있음 | steering 참조 |
| 1c 공통 모듈 | 4개 | 분할 불필요 | 소규모 |
| 02 화면 | 3개 | 플랫폼별 분할 | Android·STB / iOS / 통합 매핑 |
| 03 API/DB | 8개 | 2분할 | API / DB |
| 04 비즈니스 | 10개 | 3분할 | 규칙에러상태 / 이벤트배치CallFlow / UseCase |
| 05 보안 | 11개 | 3분할 | 크리덴셜개인정보세션 / 입력API감사암호화 / 하드닝서드파티쉴딩Layer |
| 06 품질 | 8개 | 2분할 | 테스트로깅 / 성능정합성 |
| 07 UX | 6개 | 2분할 | 인터랙션검증로딩 / 다국어딥링크피처 |
| 08 인프라 | 5개 | 2분할 | 외부연동 / 배포STB NFR |
| 09 View | 14폴더 | 4그룹 | A(컨텍스트) / B(데이터UX) / C(STB복원AI) / D(Layer Stack PM) |
| 10 Gap | 21개 | 5분할 | Part 1~5 (각 4~5개) |
| **합계** | **105개** | | |

> 📌 소규모 서비스(소스 5,000파일 미만, API 50개 미만)에서는 분할 없이 한 번에 실행 가능하다.
> 분할 실행 후 반드시 `project/artifact-checklist.md`에서 산출물 누락 여부를 확인하라.
...
4-1. **초보 개발자 친화 규칙 (인라인 해설 필수)**
   - 🔴/🟡 판정 항목에는 반드시 "왜 문제인지" 한 줄 해설을 기재하라. 판정 기호만 있고 해설이 없는 산출물은 불완전하다.
     - ❌ 나쁜 예: `| Circuit Breaker 미적용 | 🔴 Critical |`
     - ✅ 좋은 예: `| Circuit Breaker 미적용 | 🔴 Critical | 외부 시스템 장애 시 호출을 자동 차단하는 패턴이 없어, 장애가 전체 서비스로 전파될 위험 |`
   - 산출물 테이블에 기술 용어가 처음 등장할 때, 괄호 안에 한 줄 해설을 붙이거나 `(→ [용어 사전](../../glossary.md))` 링크를 추가하라.
     - 예: `mTLS(서버-클라이언트 양방향 인증서 검증) 미적용`
     - 예: `DLQ(→ [용어 사전](../../glossary.md)) 미설정`
   - Gap/이슈 리포트의 `문제 상세` 필드는 "이 코드를 처음 보는 초보 개발자도 무엇이 왜 문제인지 이해할 수 있는 수준"으로 작성하라. 전문 용어를 사용할 때는 반드시 풀어서 설명하라.
...
> Step을 잘게 나누어 컨텍스트 윈도우를 보호하는 대신, Step 간 산출물이 서로 정합성을 유지하는지
> 반드시 교차 검증하라. 각 Step 완료 시 아래 검증을 수행하고, 불일치 발견 시 즉시 보정하라.

#### 교차 검증 매트릭스

| 완료 Step | 검증 대상 (이전 Step 산출물) | 검증 내용 | 불일치 시 조치 |
|---|---|---|---|
| Step 02 (화면) | Step 01 (코드 구조) | 화면 소스 파일 경로가 01의 디렉토리 구조와 일치하는가 | 01 directory-structure.md 보정 |
| Step 03 (API/DB) | Step 02 (화면) | 화면에서 호출하는 API가 03 API 목록에 모두 존재하는가 | 03 api-endpoints.md에 누락 API 추가 |
| Step 03 (API/DB) | Step 01 (코드 구조) | 03에서 발견한 외부 의존성이 01 dependencies.md에 있는가 | 01 dependencies.md 보정 |
| Step 04 (비즈니스) | Step 02 (화면) | Use Case의 화면 ID 참조가 02 screen-inventory.md에 존재하는가 | 02에 누락 화면 추가 또는 04 UC 화면 ID 수정 |
| Step 04 (비즈니스) | Step 03 (API/DB) | Use Case의 API 엔드포인트 참조가 03에 존재하는가 | 03에 누락 API 추가 또는 04 UC API 참조 수정 |
| Step 04 (비즈니스) | Step 03 (API/DB) | 상태 전이에서 변경하는 DB 컬럼이 03 ERD에 존재하는가 | 03 erd.md 보정 |
| Step 05 (보안) | Step 03 (API/DB) | API 보안 매핑의 엔드포인트가 03 API 목록과 일치하는가 | 불일치 API 식별 후 양쪽 보정 |
| Step 05 (보안) | Step 04 (비즈니스) | 개인정보 처리 흐름이 04의 비즈니스 규칙/Use Case와 정합하는가 | 04 business-rules.md에 보안 관련 규칙 보충 |
| Step 06 (품질) | Step 03 (API/DB) | 성능 병목 후보(N+1 등)가 03 db-query-patterns.md와 일치하는가 | 양쪽 교차 보정 |
| Step 06 (품질) | Step 04 (비즈니스) | 테스트-기능 매핑의 기능 목록이 04 Use Case와 정합하는가 | Test Gap 식별 시 04 UC 참조 확인 |
| Step 07 (UX) | Step 02 (화면) | 인터랙션 패턴의 화면 ID가 02 screen-inventory.md와 일치하는가 | 02에 누락 화면 추가 또는 07 화면 ID 수정 |
| Step 07 (UX) | Step 03 (API/DB) | 딥링크 진입 경로의 API 호출이 03에 존재하는가 | 03에 누락 API 추가 |
| Step 08 (인프라) | Step 03 (API/DB) | 외부 연동 인벤토리가 03 internal-api-contracts.md와 정합하는가 | 양쪽 교차 보정 |
| Step 08 (인프라) | Step 04 (비즈니스) | 배치/스케줄러 인벤토리가 04 batch-scheduler-inventory.md와 일치하는가 | 불일치 항목 양쪽 보정 |
| Step 08 (인프라) | Step 01 (코드 구조) | 배포 토폴로지가 01 IaC/CI-CD 분석과 정합하는가 | 양쪽 교차 보정 |

#### 교차 검증 실행 절차

1. **각 Step 완료 직후**, 위 매트릭스에서 해당 Step 행의 검증을 수행하라
2. 검증 방법:
   - 이전 Step 산출물을 열어 ID/명칭/경로 등 참조값의 존재 여부를 확인
   - 수량 비교: 현재 Step에서 참조하는 항목 수 vs 이전 Step 산출물의 항목 수
   - 명칭 일관성: 동일 대상이 Step 간에 다른 이름으로 기재되어 있지 않은지 확인
3. **불일치 발견 시**:
   - 경미한 불일치(3건 이하): 즉시 해당 산출물을 열어 보정하고, `[교차검증 보정]` 태그를 붙여라
   - 중대한 불일치(4건 이상): 이전 Step 재실행을 권장하고, 작업자에게 알려라
4. 산출물 하단에 "교차 검증 로그" 테이블을 추가하라:

```markdown
---
## 교차 검증 로그

| 검증 시점 | 검증 대상 | 검증 항목 | 결과 | 불일치 내용 | 조치 |
|---|---|---|---|---|---|
| Step 03 완료 | Step 02 화면 목록 | 화면→API 참조 일치 | ⚠️ 2건 불일치 | SCR-015 화면의 /api/v2/order 미등록 | 03 api-endpoints.md에 추가 |
| Step 04 완료 | Step 03 API 목록 | UC→API 참조 일치 | ✅ 일치 | — | — |
| Step 05 완료 | Step 03 API 목록 | API 보안 매핑 일치 | ✅ 일치 | — | — |
```

#### 교차 검증 면제 조건
- Step 01, 01b, 01c는 선행 Step이 없으므로 교차 검증 면제
- 이전 Step 산출물이 아직 생성되지 않은 경우(순서 변경 실행) 면제, 단 해당 Step 완료 후 소급 검증 필수
