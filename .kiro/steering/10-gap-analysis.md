...
> 모든 Gap/문제점 항목에 소스 위치를 필수로 기재하라.
> 포맷: [source-locator-standard.md](../project/source-locator-standard.md) 참조

- Gap 리포트의 모든 항목 테이블에 `소스 위치` 컬럼을 포함하라
- 소스 없는 구간은 `<<no-source: {서비스명}>>`, `<<binary SDK: {SDK명}>>` 등으로 표기하라
- 각 Gap 항목을 `reverse-lookup-index.md`의 "9. Gap/문제점 → 소스 코드" 섹션에 등록하라

### 문제 리포트 표준 템플릿

Gap 분석에서 식별된 각 문제점은 아래 형식으로 기술하라:
...
|---|---|
| Gap ID | GAP-{서비스약어}-{순번} |
| 카테고리 | 보안 / 품질 / 성능 / 정합성 / 시나리오 / 인프라 / AI |
| 심각도 | 🔴 Critical / 🟡 Warning / 🟢 Low |
| 소스 위치 | `{Repo명}/{경로}:{라인}` |
| 관련 화면 ID | SCR-{서비스}-{번호} (해당 시) |
| 관련 API | `{HTTP메서드} {경로}` (해당 시) |
| 관련 Use Case | UC-{서비스}-{번호} (해당 시) |
| 문제 상세 | {구체적인 문제 설명} |
| | ⬆️ 작성 가이드: 초보 개발자도 이해할 수 있는 수준으로 작성하라. "무엇이 어떤 상태이고, 왜 그것이 문제이며, 방치하면 어떤 일이 생기는지"를 포함하라. 기술 용어 사용 시 괄호 안에 한 줄 해설을 붙이거나 [용어 사전](../../glossary.md) 링크를 추가하라. |
| 영향 범위 | {이 문제가 미치는 영향} |
| 개선 권고 | {권장 조치 사항} |
| 확인 상태 | ✅ / ⚠️ / ❌ / 🔍 |
| 담당 | PM / 개발 리드 / 보안센터 / 품질센터 |
```
...
| 항목 ID | 항목명 | 유형 | 해야 할 일 |
|---|---|---|---|
| P3-1 | 서비스 간 교차 검증 | `[교차검증]` | API 계약 일치, 이벤트 스키마 일치, 용어 일치 여부를 사람이 최종 검증 |
| P3-2 | 기존 요구사항-코드 매핑 확정 | `[문서수집]` `[확정]` | P0-3에서 수집한 기존 설계 문서 기반으로 요구사항↔코드 매핑을 PM/개발 리드가 확정 |
| P3-3 | 개인정보보호법 적정성 판단 | `[인터뷰]` | 법무/개인정보보호 담당자가 수집 동의, 보관 기간, 파기 절차 적정성 검토 |
| P3-4 | 보안 취약점 대응 우선순위 | `[확정]` | 보안센터가 보안 Gap 항목별 대응 우선순위 결정 |
| P3-5 | 서비스 합리화 판단 | `[확정]` | 기능 중복/저사용/고비용 서비스의 통합/폐기/재작성 여부를 PM+PO+개발 리드가 확정 |
