...
> 📖 기술 용어: [용어 사전](../../glossary.md) — ERD, ORM, gRPC, Feign, API Gateway 등
...
| Part | 산출물 | 프롬프트 예시 |
|---|---|---|
| Part 1: API | `api-endpoints.md`, `api-schema-detail.md`, `api-versioning.md`, `internal-api-contracts.md`, `service-communication-matrix.md` | `#03-api-and-data 서비스: {서비스명} 범위: Part 1 (API 엔드포인트, 스키마, 버전, 내부 계약, 통신 매트릭스)` |
| Part 2: DB | `erd.md`, `db-query-patterns.md`, `db-migration-history.md` | `#03-api-and-data 서비스: {서비스명} 범위: Part 2 (ERD, 쿼리 패턴, 마이그레이션)` |

### 분할 실행 시 주의사항
- Part 1에서 API가 참조하는 Entity/DTO를 메모해 두면 Part 2 ERD 작성이 수월하다
- 모든 Part 완료 후 `project/artifact-checklist.md`의 Step 03 섹션에서 8개 산출물 생성 여부를 확인하라
- API 수가 50개 미만인 소규모 서비스에서는 분할 없이 한 번에 실행 가능
...
- [ ] **Step 01 → code-structure.md 업데이트 필요?**
  ...
  - 검증 방법: `grepSearch`로 새로 발견한 Service/Client 클래스명을 `01-code-structure/directory-structure.md`에서 검색. 없으면 추가 필요
- [ ] **Step 02 → screen-inventory.md 업데이트 필요?**
  ...
  - 검증 방법: API 엔드포인트 URL을 `02-screens/screen-inventory.md`에서 검색하여 매핑 누락 확인
- [ ] **Step 01c → common-module 그룹핑 업데이트 필요?**
  - 여러 API에서 공통으로 사용하는 DTO/모델이 공통 모듈로 분류되어 있는지 확인
  - 검증 방법: 3개 이상 Controller에서 import하는 DTO/모델을 `01c-common-modules/internal-common-modules.md`에서 검색
...
- 다음: [Step 04 — 비즈니스 로직 추출](04-business-logic.md)
- 이전: [Step 02 — 화면 목록 추출 및 화면명 정규화](02-screen-inventory.md)
